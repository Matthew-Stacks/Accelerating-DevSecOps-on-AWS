...
def get_description_of_rule(config_rule_name):
    ...
    try:
        ...
        if 'Description' in response['ConfigRules'][0]:
            return response['ConfigRules'][0]['Description']
        else:
            return response['ConfigRules'][0]['ConfigRuleName']
    except Exception as error:
        print("Error: ", error)
        raise
        
def get_compliance_and_severity(new_status):
    return ['PASSED', 0, 0] if new_status == 'COMPLIANT' else ['FAILED', 3.0, 30]
...
def map_config_findings_to_sh(args):
    ...
    remediation_url = f"https://console.aws.amazon.com/config/home?region={region}#/rules/rule-details/{config_rule_name}"
